...
export function CollectionShareButton({
  ...
  const shareText = moodInfo
    ? `Check out my "${collectionName}" collection ${moodInfo.emoji} - ${moodInfo.label} vibes!`
    : `Check out my "${collectionName}" movie collection!`;
      ...
      <PopoverContent className="w-56 p-2 bg-card border-border" align="end">
        <div className="space-y-1">
          ...
          >
            ...
            <span className="text-sm text-foreground">Share on X</span>
          </button>
          ...
          <button
            onClick={() => {
              ...
              handleCopyLink();
              toast({ title: "Link copied! Paste it in your Instagram story or DM" });
              setIsOpen(false);
            }}
            className="flex items-center gap-3 w-full px-3 py-2 rounded-lg hover:bg-secondary transition-colors text-left"
          >
            ...
            <span className="text-sm text-foreground">Share on Instagram</span>
          </button>

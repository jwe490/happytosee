...
export const moods: MoodOption[] = [
  ...
  },
  {
    ...
    emoji: "🥹",
    label: "Nostalgic",
    description: "Feeling sentimental",
    gradientFrom: "from-purple-400/20",
    gradientTo: "to-violet-400/20",
  },
  {
    id: "relaxed",
    emoji: "😌",
      ...
      {moods.map((mood, index) => {
        ...
        return (
          ...
          >
            {/* Gradient Background */}
            <div
              className={`absolute inset-0 bg-gradient-to-br ${mood.gradientFrom} ${mood.gradientTo} opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isSelected ? "opacity-100" : ""}`}
            />
            ...
            <div className="relative flex flex-col items-center gap-3 md:gap-4">
              ...
              <div className="text-center space-y-1">
                <h3 className="font-display font-semibold text-sm md:text-base text-foreground">{mood.label}</h3>
                <p className="text-[10px] md:text-xs text-muted-foreground hidden sm:block">{mood.description}</p>
              </div>
